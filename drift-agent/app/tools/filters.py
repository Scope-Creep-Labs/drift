...
FILTERS_TOOLS: list[dict] = [
    ...
    },
    {
        ...
        "description": (
            ...
            "Returns BOTH the calling operator's private filters AND every fleet-wide filter "
            "(regardless of original creator). ALWAYS call this at the START of ANY "
            "investigation that may surface errors, alerts, or noisy logs — INCLUDING "
            "fleet-wide questions like 'how's the fleet doing?' or 'any errors?'. Pass "
            "whichever scope keys are known (device, container, group, signal); for a "
            "fleet-wide question pass `{}` and you get every visible filter. The server "
            "returns every filter whose scope is compatible with yours (including filters "
            "that pin EXTRA keys you didn't pass — those still apply on a per-line basis "
            "to lines whose source matches). For each returned filter, apply it PER LINE: "
